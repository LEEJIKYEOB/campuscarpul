...
public class Join01Activity extends ImguploadActivity implements OnClickListener, OnHttpReceiveListener {
    ...
    public void Http_Receive(Message msg) {
        ...
        switch (msg.arg1) {
            ...
            case HTTP_Handler.HTTP_RECEIVE_JOIN:
                ...
                if (LoginActivity.Act != null) {
                    LoginActivity.Act.finish();
                }


                Static_date.carpopup = true;

                Toast.makeText(getApplicationContext(), "회원가입 완료! 환영합니다. " + userProfile_bean.getName() + "님.", Toast.LENGTH_SHORT).show();

                Static_date.loginpross(this, id, pw, true);
            ...
            case HTTP_Handler.HTTP_RECEIVE_FILEUPLOAD:
                ...
                userProfile_bean.setDevicekey(Static_date.myProfile.getDevicekey());
...
//		}
        ...
        if (img_src != null) {
            FILE_UPLOAD_HTTP upload_HTTP = new FILE_UPLOAD_HTTP(handler, HTTP_Handler.HTTP_RECEIVE_FILEUPLOAD);
            upload_HTTP.start();
        } else {
